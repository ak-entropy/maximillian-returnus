...
ST = {
    ...
    "last_poll": None,
    "poll_count": 0,
    "assets": list(DEFAULT_ASSETS),   # every asset is tracked independently, same strategy
    "settings": {
    ...
    "current": {},              # { asset: snapshot } for the active window
    "windows_observed": {},     # { asset: count }
    "errors": deque(maxlen=50),
}

# entry tracking: {(asset, window_ts): trade_id} so we never double-enter a market
_entered_windows = {}
_known_window = None
...
# Core poll
# --------------------------------------------------------------------------- #
async def poll_asset(asset: str, wts: int, now: float):
    """Poll one market and (paper-)enter if the rule fires. Returns the snapshot."""
    s = ST["settings"]
    ...
    if ev:
        ...
        already = (asset, wts) in _entered_windows
        cur["entered"] = already
        ...
        if (not already and up_mid is not None and seconds_into <= s["entry_cutoff"]):
            ...
            if side:
                ...
                if tid:
                    _entered_windows[(asset, wts)] = tid
                    cur["entered"] = True
                    push_log(
                        f"{asset.upper()} ENTRY: UP mid {up_mid:.2f} -> BUY {side} ({outcome}) "
...
async def poll_once():
    ...
    # new window? (all 5-min markets roll on the same UTC boundary)
    if wts != _known_window:
        _known_window = wts
        ts_label = datetime.fromtimestamp(wts, timezone.utc).strftime('%H:%M:%S')
        push_log(f"New window @ {ts_label} UTC — {len(ST['assets'])} markets")
        for a in ST["assets"]:
            ST["windows_observed"][a] = ST["windows_observed"].get(a, 0) + 1

    # poll each market in turn (sequential keeps SQLite writes simple & safe)
    markets = {}
    for asset in ST["assets"]:
        try:
            cur = await poll_asset(asset, wts, now)
        except Exception as e:
            ST["errors"].append(f"poll {asset}: {e}")
            cur = {"asset": asset, "slug": slug_for(wts, asset), "found": False}
        ST["current"][asset] = cur
        markets[asset] = {"current": cur, "stats": compute_stats(asset)}

    await resolve_pending()
    await _broadcast({"type": "state", "markets": markets, "running": ST["running"]})
...
async def resolve_pending():
    ...
    for row in rows:
        ...
        key = (row["asset"], row["window_ts"])
        if winner in ("UP", "DOWN"):
            win = resolve_trade(row["id"], winner)
            push_log(
                f"{row['asset'].upper()} RESOLVED: winner {winner} -> "
                f"{'WIN ✅' if win else 'LOSS ❌'} (bought {row['bought_outcome']})",
                level="win" if win else "loss",
            )
            _entered_windows.pop(key, None)
        elif now > close_ts + VOID_AFTER_SECS:
            void_trade(row["id"])
            push_log(f"{row['asset'].upper()} VOID: no resolution after 30m", level="warn")
            _entered_windows.pop(key, None)


def compute_stats(asset: str | None = None):
    with db() as conn:
        if asset:
            rows = conn.execute("SELECT * FROM trades WHERE asset=?", (asset,)).fetchall()
        else:
    ...
    return {
        ...
        "windows_observed": ST["windows_observed"].get(asset, 0) if asset else sum(ST["windows_observed"].values()),
    }


def all_stats():
    return {a: compute_stats(a) for a in ST["assets"]}
...
async def _startup():
    ...
    # Auto-resume so the harness runs 24/7 (survives server reboots). Set MAX_AUTOSTART=0 to disable.
    if os.environ.get("MAX_AUTOSTART", "1") != "0":
        ST["running"] = True
        ST["started_at"] = datetime.now(timezone.utc).isoformat()
        _poller_task = asyncio.create_task(poller())
        log.info("Autostart enabled — poller running")
...
async def health():
    return {
        ...
        "markets": {a: {"current": ST.get("current", {}).get(a, {}),
                        "stats": compute_stats(a)} for a in ST["assets"]},
...
async def tick(authorization: str | None = Header(None)):
    ...
    require_auth(authorization)
    await poll_once()
    return {"markets": {a: {"current": ST.get("current", {}).get(a, {}),
                            "stats": compute_stats(a)} for a in ST["assets"]}}
...
async def state():
    return {"markets": {a: {"current": ST.get("current", {}).get(a, {}),
                            "stats": compute_stats(a)} for a in ST["assets"]},
            "running": ST["running"], "settings": ST["settings"], "assets": ST["assets"]}


@app.get("/trades")
async def trades(limit: int = 500, asset: str | None = None):
    with db() as conn:
        if asset:
            rows = conn.execute(
                "SELECT * FROM trades WHERE asset=? ORDER BY window_ts DESC LIMIT ?",
                (asset, limit)).fetchall()
        else:
            rows = conn.execute(
                "SELECT * FROM trades ORDER BY window_ts DESC LIMIT ?", (limit,)).fetchall()
    return JSONResponse([dict(r) for r in rows])


@app.get("/stats")
async def stats():
    return {"overall": compute_stats(), "by_asset": all_stats()}
...
async def update_settings(payload: dict, authorization: str | None = Header(None)):
    ...
    for k in ("low_thresh", "high_thresh", "stake", "poll_interval", "entry_cutoff"):
        if k in payload:
            s[k] = float(payload[k])
    if "assets" in payload and isinstance(payload["assets"], list) and payload["assets"]:
        ST["assets"] = [str(a).lower() for a in payload["assets"]]
    push_log(f"Settings updated: {s} assets={ST['assets']}", level="warn")
    return {"settings": s, "assets": ST["assets"]}
